...
@section('content')
    <table class="generalTable">
        ...
        <tr>
            ...
            <td><a class="nameTableProfile" href="{{route('rights.admin.edit',['id' => $user->id])}}">{{$user->name}} {{$user->lastname}}</a></td>
            <td>{{$user->email}}</td>
            <td>
            ...
            <td>
                <form action="{{route('adminrights.switchToUser',['id' => $user->id])}}" method="Post">
                    @method('PUT') @csrf
                    <select onchange="document.getElementById('{{'subSelect'.$user->id}}').click()">
                        <option>User</option>
                        <option selected="selected">Admin</option>
                    </select>
                    <input id="{{'subSelect'.$user->id}}" style="display: none" type="submit">
                </form>
            </td>
            <td class="editColumn">
<!--                <i class="editIcon">{{__('users.rights.edit')}}</i>
                ...
                <div class="circleMenuIcon" onclick="toggleCircleMenu(event)">
                    <div class="circleMenu">
                        <a class="circleMenuOption" href="{{ route('rights.admin.edit',['id' => $user->id] ) }}"><i class="circleMenuRowIcon editIconWhite"></i>{{__('users.rights.edit')}}</a>
                        <form class="circleMenuOption" action="{{route('admin.delete',['id' => $user->id])}}" method="Post" onclick="submitDelete(event)">
                            @method('DELETE') @csrf
                ...
                <a class="editIcon editIconEdiTable" href="{{ route('rights.admin.edit',['id' => $user->id] ) }}"></a>
        ...
        <div class="paginationCenter">
            ...
            @if($pagination <= 0)
                @for($x = $pagination; $x < $pagination+8;$x++)
                    <a href="{{route('rights.admin.index',$x)}}">{{$x}}</a>
                @endfor
            @else
                ...
                @endfor
            @endif
            <a id="arrowRight" class="arrowIcon" href="{{ route('rights.admin.index',++$pagination)}}"></a>
        </div>
    </div>
@endsection
