    ...
    <div #shareModal class="modalWindowWrapper">
      <div class="modalWindow">
        ...
        <div class="buttonVersioningWrapper">
          <button class="buttonSuccess" #submitBtn (click)="shareFolderSubmit()" disabled i18n>Share</button>
  ...
  export class SharefolderComponent
{
  ...
  constructor(private drive: DriveService,private router: Router,private login: LoginService,private share: SharedService) {
    ...
    this.closed = true;
    this.selectedUser = {};
    this.shared = {uploaded: false};
    this.folderToInstantiate = {};
  ...
  async shareFolderSubmit()
  {

    if(this.selectedUserType?.nativeElement?.value == 2) {
      this.drive.makeSharedFolder(
    ...
    while(!this.shared.uploaded)
    {
      await new Promise(resolve => setTimeout(resolve, 250));
    }

   /*
    let folders = this.share.getObject("shareddocument").folders;

    if(folders[folders.length-1 < 0 ? 0 : folders.length-1])
    {
      folders[folders.length-1 < 0 ? 0 : folders.length-1].push(this.folderToInstantiate);
    }
    else {
      folders.push([this.folderToInstantiate]);
    } */
  ...
  queryUser(input:string)
  {
    ...
    {
      url = "mydocument";
      this.drive.getUserByHttp(url,this.queriedUser,
        input,
